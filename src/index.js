...
import { syncSlashCommandsFromClient } from "./lib/command-sync.js";
...
client.on("ready", async () => {
  ...
  const shouldSyncCommands = process.env.AUTO_REGISTER_COMMANDS !== "0";
  if (!shouldSyncCommands) {
    logger.info("AUTO_REGISTER_COMMANDS=0, skipping startup slash-command sync.");
    return;
  }

  try {
    await syncSlashCommandsFromClient({
      client,
      guildId: process.env.GUILD_ID,
      scope: process.env.COMMAND_SYNC_SCOPE || "guilds",
      commands: commandPayload,
      logger,
    });
  } catch (error) {
    logger.error("Failed to sync slash commands on startup.", error);
  }
});
